...
# CC_FOR_TARGET is required for the ld testsuite.
	...
	$(MAKE) -j $(FORKS) -C $(dir $@) &>$(dir $@)/make-build.log
	$(MAKE) -j $(FORKS) -C $(dir $@) install install-pdf install-html &>$(dir $@)/make-install.log
	date > $@
...
$(OBJDIR)/%/build/riscv-gnu-toolchain/build-gcc-newlib-stage1/stamp: \
	...
	$(MAKE) -j $(FORKS) -C $(dir $@) all-gcc &>$(dir $@)/make-build.log
	$(MAKE) -j $(FORKS) -C $(dir $@) install-gcc &>$(dir $@)/make-install.log
	date > $@
...
$(OBJDIR)/%/build/riscv-gnu-toolchain/build-newlib/stamp: \
	...
	$(MAKE) -j $(FORKS) -C $(dir $@) &>$(dir $@)/make-build.log
	$(MAKE) -j $(FORKS) -C $(dir $@) install &>$(dir $@)/make-install.log
...
$(OBJDIR)/%/stamps/expat/install.stamp: \
	...
	$(MAKE) -j $(FORKS) -C $($@_BUILD) buildlib &>$($@_BUILD)/make-buildlib.log
	$(MAKE) -j $(FORKS) -C $($@_BUILD) installlib &>$($@_BUILD)/make-installlib.log
	mkdir -p $(dir $@)
	date > $@
...
$(OBJDIR)/%/build/riscv-openocd/riscv-openocd/stamp: \
	...
	$(MAKE) -j $(FORKS) $($($@_TARGET)-rocd-vars) -C $(dir $@) &>$(dir $@)/make-build.log
	$(MAKE) -j $(FORKS) $($($@_TARGET)-rocd-vars) -C $(dir $@) pdf html &>$(dir $@)/make-build-doc.log
	$(MAKE) -j $(FORKS) $($($@_TARGET)-rocd-vars) -C $(dir $@) install install-pdf install-html &>$(dir $@)/make-install.log
	date > $@
...
$(OBJ_WIN64)/build/riscv-qemu/zlib/stamp: \
	...
	$(MAKE) -j $(FORKS) -C $(dir $@) -f win32/Makefile.gcc PREFIX=$(WIN64)- prefix=$(abspath $(OBJ_WIN64)/install/riscv-qemu-$(RQEMU_VERSION)-$(WIN64))/ &>$(dir $@)/make-build.log
	$(MAKE) -j $(FORKS) -C $(dir $@) -f win32/Makefile.gcc SHARED_MODE=1 install DESTDIR=$(abspath $(OBJ_WIN64)/install/riscv-qemu-$(RQEMU_VERSION)-$(WIN64))/ INCLUDE_PATH=include LIBRARY_PATH=lib BINARY_PATH=bin &>$(dir $@)/make-install.log
	date > $@
...
$(OBJDIR)/%/build/riscv-qemu/riscv-qemu/stamp: \
	...
	$($($@_TARGET)-rqemu-vars) $(MAKE) -j $(FORKS) -C $(dir $@) install &>$(dir $@)/make-install.log
	date > $@
...
$(OBJDIR)/%/build/trace-decoder/trace-decoder/stamp: \
	...
	$(MAKE) -j $(FORKS) -C $(dir $@) CROSSPREFIX=$($($@_TARGET)-tdc-cross) all &>$(dir $@)/make-build.log
...
$(OBJDIR)/%/build/sdk-utilities/dtc/stamp: \
	...
	$(MAKE) -j $(FORKS) -C $(dir $@) install PREFIX=$(abspath $($@_INSTALL)) $($($@_TARGET)-dtc-configure) \
...
$(OBJDIR)/%/build/sdk-utilities/freedom-elf2hex/stamp: \
	...
	$(MAKE) -j $(FORKS) -C $(dir $@) install INSTALL_PATH=$(abspath $($@_INSTALL)) $($($@_TARGET)-fe2h-configure) &>$(dir $@)/make-install.log
	date > $@
